<template>
    <div class="goodsinfo-container">
        ...
        <transition @before-enter="beforeEnter" @enter="emter" @after-enter="afterEnter">
            ...
            <div class="mui-card-content">
                <div class="mui-card-content-inner">
                    ...
                    <p>
                        ...
                        <!-- 1.经过分析发现：按钮属于goodslist页面，数字属于numberbox组件 -->
                        <!-- 2.由于涉及到了父子组件的嵌套了，所以无法直接在 goodsinfo页面中获取到选中的商品数量值 -->
                        <!-- 3.怎么解决这个问题：涉及到了子组件向父组件传值了（事件调用的机制） -->
...
export default {
    ...
    methods:{
        ...
        enter(el,done){
            ...
            // 2.只要分辨率和测试的时候不一样，或者滚动条有一定的滚动 的距离之后，问题就出现了；
            // 3.因此，我们分析得出结论：不能把位置的横纵坐标直接写死了，而是应该根据不同情况，动态计算这个坐标值；
            // 4.经过分析，得出解题思路：先得到徽标的横纵坐标；再得到小球的横纵坐标，然后，让y值求差，x值也求差，得到的结果就是横纵坐标要位移的距离
        ...
        getSelectedCount(count){
            // 当子组件把选中的数量传递给父组件的时候，把选中保存到data上
